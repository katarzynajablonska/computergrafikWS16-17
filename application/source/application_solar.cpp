 ...
 ,planet_object{}
{
  ...
  int vector_size = rand() % 6 + 5;
  std::vector<light_struct> lights(vector_size);
  update_textures();
  initializeGeometry();
...
void ApplicationSolar::keyCallback(int key, int scancode, int action, int mods)
{
  ...
  else if (key == GLFW_KEY_7 && action == GLFW_PRESS)
  {
      ...
      if (!is_greyscale)
      {
          glUniform1i(glGetUniformLocation(m_shaders.at("screenquad").handle, "greyscale"), 1);
          is_greyscale = true;
      }
      else
      {
          glUniform1i(glGetUniformLocation(m_shaders.at("screenquad").handle, "greyscale"), 0);
          is_greyscale = false;
      }
  }
  else if (key == GLFW_KEY_8 && action == GLFW_PRESS)
  {
      ...
      if (!is_mirror_h)
      {
          glUniform1i(glGetUniformLocation(m_shaders.at("screenquad").handle, "mirror_horiz"), 1);
          is_mirror_h = true;
      }
      else
      {
          glUniform1i(glGetUniformLocation(m_shaders.at("screenquad").handle, "mirror_horiz"), 0);
          is_mirror_h = false;
      }
  }
  else if (key == GLFW_KEY_9 && action == GLFW_PRESS)
  {
      ...
      if (!is_mirror_v)
      {
          glUniform1i(glGetUniformLocation(m_shaders.at("screenquad").handle, "mirror_vert"), 1);
          is_mirror_v = true;
      }
      else
      {
          glUniform1i(glGetUniformLocation(m_shaders.at("screenquad").handle, "mirror_vert"), 0);
          is_mirror_v = false;
      }
  }
  else if (key == GLFW_KEY_0 && action == GLFW_PRESS)
  {
      ...
      if(!is_gausblur)
      {
          glUniform1i(glGetUniformLocation(m_shaders.at("screenquad").handle, "gaus_blur"), 1);
          is_gausblur = true;
      }
      else
      {
          glUniform1i(glGetUniformLocation(m_shaders.at("screenquad").handle, "gaus_blur"), 0);
          is_gausblur = false;
      }
  }
  else
...
void ApplicationSolar::initializeShaderbuffers() const
{
  GLuint block_index;
  glUseProgram(m_shaders.at("planet").handle);
  block_index = glGetProgramResourceIndex(m_shaders.at("planet").handle, GL_SHADER_STORAGE_BLOCK, "lights");
  glShaderStorageBlockBinding(m_shaders.at("planet").handle, block_index, bindingPoint_ssbo);

  glGenBuffers(1, &ssbo);
  glBindBuffer(GL_SHADER_STORAGE_BUFFER,ssbo);
  glBufferData(GL_SHADER_STORAGE_BUFFER, 2*sizeof(float)*3, 0, GL_STREAM_DRAW);

  glBindBufferBase(GL_SHADER_STORAGE_BUFFER, bindingPoint_ssbo, ssbo);
    
  glBindBuffer(GL_SHADER_STORAGE_BUFFER, ssbo);
  glBufferSubData(GL_SHADER_STORAGE_BUFFER, 0, sizeof(float)*3, glm::value_ptr(light_struct_data.ambient_app));
  glBufferSubData(GL_SHADER_STORAGE_BUFFER, sizeof(float)*3, sizeof(float)*3, glm::value_ptr(light_struct_data.diffuse_app));
  glBindBufferRange(GL_SHADER_STORAGE_BUFFER, bindingPoint_ssbo, ssbo, 0, sizeof(float)*3*9);
}
